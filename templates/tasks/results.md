# Results: TASK-####

## Packet State
- **Current Task Status:** [in_progress / blocked / review]
- **Review Readiness:** [ready / blocked / needs fixes]
- **Recommended Next Status:** [review / blocked / in_progress]

## Files Changed
- [path] — [description of change]

## Summary
[What was implemented. What decisions were made. Any deviations from the plan.]

## Test Results
[N/N new tests passing. N/N total passing.]

## Efficiency
- **Prompt Runs:** [count for this task workflow so far]
- **Conversation Restarts:** [count]
- **Files Read (estimated):** [count]
- **Exact Tokens:** [input/output/total if available, otherwise "not available"]
- **Efficiency Notes:** [where token usage stayed low, where waste occurred, or "None"]

## Review Notes
- [key fact a reviewer should verify]
- [drift, ambiguity, or tradeoff to inspect]

## Review Intake
- **Review Decision:** [ready / needs fixes / blocked / unclear due to spec conflict]
- **Definition of Done Met:** [yes / no]
- **Recommended Next Status:** [review / in_progress / blocked / done]

### Required Fixes
- [fix, or "None"]

### Open Questions To Log
- [question summary, or "None"]

### Proposal Candidates To Log
- [proposal summary, or "None"]

### Follow-Ups To Log
- [follow-up note, or "None"]

### Residual Risks
- [risk, or "None"]

## Deliverable Checklist
- [ ] [criterion from deliverable_spec.md]
- [ ] All tests passing

## Blockers
[None. / Description of any unresolved blocker.]
