# Deliverable Spec: P35-T10 — Version check command

## Required Output

### New Files
- `tasks/P35-T10/task.md` — packet metadata/scope ✓
- `tasks/P35-T10/deliverable_spec.md` — this file ✓
...
- `src/grain/domain/capabilities.py` — the `version_check` `Capability` is **seeded and
  owned by P35-T03** (with `surfaces ∋ "cli"`/`"mcp"` pinned); this packet consumes it and
  does NOT add or edit the entry.
- The MCP `_ok` envelope helper and the extended `McpTool` (`write`+`capability`) from
  **P35-T06** — consumed to register the tool; T06 does not register `version_check` itself.
...
## Capability registry entry — CONSUMED from P35-T03 (`domain/capabilities.py`, §6.2)
**Do NOT add or edit this entry — P35-T03 seeds and owns it.** It is reproduced here only
...
## MCP tool — OWNED here, moved out of P35-T06 (`services/mcp_service.py`, §5.4, §5.7)
- Register a **read** tool `version_check` (`write=False`, `capability="version_check"`)
  using the **`McpTool` (`write`+`capability`) and `_ok` helper delivered by P35-T06**.
...
- The envelope dataclass, `ErrorEnvelope`, `to_envelope`/`envelope_to_dict`, and
  `get_version()` themselves (delivered by P35-T01 / P35-T02 — consumed here, not built).
...
- [ ] `version_check` present in `CAPABILITIES` (seeded by P35-T03; `since 0.5.0`, read,
