...
today); P37-T06 makes `TOOLS`/`tools/list` a derived view of the registry, so the wrapper's
...
view (it iterates `mcp_service.TOOLS`), so it needs no fix — once P37-T06 makes `TOOLS` registry-
...
- EDIT `apps/grain-mcp/main.py`:
  ...
  - **JSON-RPC protocol errors → §4 error shape at the wrapper boundary (§5.6).** JSON-RPC
    *transport* errors stay protocol-level/unchanged inside `mcp_service` (§4.3); the mapping to the
    ...
    serializer from P37-T01 if conveniently importable, §4.2).
  - **Version from `get_version()` (§5.2).** Replace the hardcoded FastAPI `version="0.1.0"` with
    `get_version()` (the single resolver from P37-T02). **`GET /mcp/tools` needs no change** — it
    already derives from `mcp_service.TOOLS` (§5.3 derived view); once P37-T06 makes `TOOLS`
...
- A JSON-RPC **protocol** error (e.g. unknown method or invalid params) yields a body whose `error`
  ...
  upstream (§4.3).
- The FastAPI app's advertised `version` equals `get_version()` (P37-T02); `grep` finds no
...
## Dependencies
- **P37-T01** — provides the shared `domain/envelope.py` (`ErrorEnvelope` + `to_envelope`/
...
- **P37-T02** — provides `grain/version.py::get_version()`, the single version source the wrapper
  must use for its advertised version and `serverInfo`.
- **P37-T06** — expands `mcp_service` to emit the `grain.engine/v1` envelope
...
- `products/grain/src/grain/services/mcp_service.py` (reference, not edited here) — `handle_request`,
  ...
  P37-T06).
- `products/grain/src/grain/version.py` (reference, from P37-T02) — `get_version()`.
- `products/grain/src/grain/domain/envelope.py` (reference, from P37-T01) — §4.2 `ErrorEnvelope` /
...
- If P37-T06 has not yet landed the enveloped MCP result / registry-derived `tools/list`, stop and
