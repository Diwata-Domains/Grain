...
## Dependencies
- **P35-T01** (envelope + typed-error foundation) — the `grain.engine/v1` frame + taxonomy under test.
- **P35-T02** (version resolver) — `get_version()` referenced by the parity/version tests.
- **P35-T03** (capability registry) — the registry the §10 group registration ratifies.
- **P35-T04** (envelope CLI wiring + opt-in flag) — the CLI json envelope the tests parse.
- **P35-T05** (`engine_error_to_forge` extraction) — recipe-error parity across CLI/MCP.
- **P35-T06** (MCP surface expansion) — the MCP side of every cross-surface assertion.
- **P35-T07** (HTTP wrapper pass-through fix) — the §5.6 retreat recorded in the proposal.
- **P35-T08** (capability file writer + capabilities/workspace CLI) — the §6 surface ratified.
- **P35-T09** (non-interactive `--yes` / `gate()`) — the `docs audit --fix` + gate behaviour tested.
- **P35-T10** (`grain version` + `--check`) — the fail-silent behaviour tested.
...
- `tests/test_engine_envelope.py` (reference, P35-T01) — tri-state / round-trip patterns to extend.
...
- `src/grain/adapters/pypi.py` (reference, P35-T10) — the `fetch_latest` to monkeypatch.
