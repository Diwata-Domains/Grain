# Deliverable Spec: P35-T07 — HTTP wrapper pass-through fix (no auth)
...
### New Files
- `tasks/P35-T07/task.md` — packet metadata/scope ✓
- `tasks/P35-T07/deliverable_spec.md` — this file ✓
...
- `products/grain/src/grain/services/mcp_service.py` — envelope + registry-derived `TOOLS`/
  `tools/list` are delivered by P35-T06; this packet relays them, it does not build them.
- `products/grain/src/grain/version.py` — `get_version()` is delivered by P35-T02.
- `products/grain/src/grain/domain/envelope.py` — §4.2 `ErrorEnvelope`/`envelope_to_dict` delivered
  by P35-T01; the boundary error map mirrors that shape.
...
- The returned `result` MUST retain, exactly as produced by `mcp_service` (P35-T06):
...
- The listing **already** iterates `mcp_service.TOOLS` (today's `main.py` builds it from
  `for t in TOOLS`). P35-T06 makes `TOOLS`/`tools/list` a derived view of `CAPABILITIES`, so the
...
- Promoting the HTTP wrapper to a canonical contract surface; a "control vs data channel"
  distinction.
- Modifying `mcp_service.py` envelope construction or the registry (P35-T06's job).
- Modifying `get_version()` (P35-T02's job).
