...
- **Engine-contract:** capability discovery layer — implements the on-disk
  ...
  are upstream foundation packets (P37-T03, P37-T02); this packet consumes them, it does
...
consumer half of the §6 foundation (P37-T03 owns the registry, P37-T02 owns
...
- **New `src/grain/services/capabilities_service.py`** (spec §6.3, §10):
  - `write_capabilities_file(root) -> Path` — serialize `CAPABILITIES` (from
    `domain/capabilities.py`, P37-T03) + `get_version()` (from `grain/version.py`,
    P37-T02) to `<root>/docs/runtime/grain_capabilities.yaml` at the **exact locked
...
- Reuse Grain idioms: new `.py` files carry the SPDX Apache-2.0 header (Grain was
  ...
  `Capability` dataclass + `VALID_*` frozensets it depends on are owned by P37-T03 and
...
- **Drift is the only failure axis here.** `capabilities --check` drift → `ConfigError`
  ...
  exit-code mapping is owned by P37-T01 and must not change.
...
## Dependencies
- **P37-T01** — `domain/envelope.py::EngineEnvelope` (+ serializer) and the §4 error
...
- **P37-T02** — `grain/version.py::get_version()` (the single version resolver; supplies
  the file's `grain_version` and the drift comparison). Spec §5.2, §10.
- **P37-T03** — `domain/capabilities.py::CAPABILITIES` + the `Capability` dataclass and
...
- `src/grain/cli/doctor.py` / `src/grain/services/doctor_service.py` (edit) — drift
  warning check.
- `src/grain/domain/capabilities.py` (reference, from P37-T03) — `CAPABILITIES` +
  `Capability` shape this packet serializes.
- `src/grain/version.py` (reference, from P37-T02) — `get_version()`.
...
- If the `Capability` dataclass shape from P37-T03 lacks a field this writer must
  serialize (e.g. `drive`/`stability`/`surfaces`), stop and record a change proposal
  against P37-T03 rather than re-defining the registry here (the registry is owned there,
  spec §6.1).
...
mechanical given the registry (P37-T03) and version resolver (P37-T02) are already
