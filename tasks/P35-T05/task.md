...
- Tests in `tests/` (e.g. `tests/test_engine_error_to_forge.py`): assert each engine error
  ...
  (from P35-T01 ClassVars), and that the catch-all routes an unmapped/future
...
- **No taxonomy changes.** Do not add/rename `ForgeError` subclasses, error `code` strings,
  or exit codes — those are owned by P35-T01 (§4.1/§4.2). This packet only *maps onto* the
  existing taxonomy.
...
- **No `data`-shape or envelope work.** This packet does not touch `EngineEnvelope`,
  ...
  consume. (Envelope/error object delivered by P35-T01.)
...
## Dependencies
- **P35-T01** (envelope + typed errors) — provides the `ForgeError` `code`/`exit_code`
...
- `src/grain/domain/errors.py` — `ForgeError` taxonomy (`MissingPathError`, `UsageError`,
  `ValidationError`); the `code`/`exit_code` ClassVars come from P35-T01.
...
- If P35-T01 has not landed the `ForgeError` `code`/`exit_code` ClassVars, the function can
