# Deliverable Spec: P35-T04 — Envelope CLI wiring + opt-in flag

## Required Output

### New Files
- `tasks/P35-T04/task.md` — packet metadata/scope ✓
- `tasks/P35-T04/deliverable_spec.md` — this file ✓
...
- `src/grain/domain/envelope.py` (P35-T01) — `EngineEnvelope`, `VALID_ENGINE_KINDS`.
...
- `src/grain/version.py` (P35-T02) — `get_version()`, the single source for the frame's
...
```python
import dataclasses
from grain.domain.envelope import EngineEnvelope   # from P35-T01 (frame only; NOT envelope_to_dict)
from grain.version import get_version              # from P35-T02 — §5.2/§10 single source; never hardcode

def emit(
    ctx,
    kind: str,                       # ∈ VALID_ENGINE_KINDS (P35-T01)
...
- **Wrapped path** (`enveloped is True`): construct
  ```python
  EngineEnvelope(
      apiVersion="grain.engine/v1",   # constant from P35-T01, NOT grain-kit version (§3.1)
...
All wired kinds MUST already be members of `VALID_ENGINE_KINDS` in P35-T01. If any is
...
- Each wired command sets `ctx.obj["kind"] = "<its kind>"` at the top of its body (before
  any work that could raise), so a downstream format-aware `handle_error` (P35-T02) can read
...
- The format-aware `handle_error` error envelope itself (P35-T02) — this packet only
