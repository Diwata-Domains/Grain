...
- **Backlog:** P35-T04
- **Packet Path:** tasks/P35-T04/
- **Dependencies:** P35-T01, P35-T02
- **Primary Adapter:** code
- **Secondary Adapters:** none
- **Engine-contract:** envelope wiring layer — consumes `domain/envelope.py` (P35-T01)
  and `grain/version.py::get_version()` (P35-T02), and threads the `grain.engine/v1`
...
§2 principle 2, §3). P35-T01 lands the `EngineEnvelope` dataclass + serializers in
...
format-aware error path (P35-T02/handle_error) nor the always-enveloped new commands can
...
- **The single emit helper** (new, e.g. `cli/emit.py::emit(ctx, kind, data, *, warnings=())`):
  - Reads `ctx.obj["envelope"]` (bool). If `False`: `click.echo(json.dumps(data, indent=2))`
    ...
    `get_version()` (P35-T02), `command` annotation, `data`, `warnings`), then build the
...
- Consume `EngineEnvelope`/serializers/`VALID_ENGINE_KINDS` from `domain/envelope.py`
  (P35-T01); do NOT redefine the frame or the kind set here (§2 principle 2, §10 "none
  re-defines a frame").
...
## Dependencies
- **P35-T01** (envelope + errors foundation) — provides `domain/envelope.py`:
...
- **P35-T02** (version resolver) — provides `grain/version.py::get_version()`, the single
...
- `src/grain/domain/envelope.py` (dependency, P35-T01) — `EngineEnvelope`,
...
- `src/grain/version.py` (dependency, P35-T02) — `get_version()` for `grain_version`
...
- If a `workflow`/`recipe` emit site's current `data` shape is not in the §3.2 `kind`
  ...
  programming error).
- If P35-T01's serializer signature or `EngineEnvelope` field set differs from §3.1, do
