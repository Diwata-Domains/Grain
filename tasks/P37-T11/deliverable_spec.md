# Deliverable Spec: P37-T11 — Change proposal + conformance tests + docs

## Required Output

### New Files
- `tasks/P37-T11/task.md` — packet metadata/scope ✓
- `tasks/P37-T11/deliverable_spec.md` — this file ✓
...
- `from grain.domain.envelope import EngineEnvelope, ErrorEnvelope, to_envelope, envelope_to_dict,
  VALID_ENGINE_KINDS, VALID_ERROR_CODES` (P37-T01) — frame + error object under test.
- `from grain.domain.errors import (the 7 ForgeError subclasses)` (P37-T01) — taxonomy under test.
- `from grain.version import get_version` (P37-T02) — version source for parity/version tests.
- MCP entry points in `src/grain/services/mcp_service.py` (P37-T06) — `tools/call` handler +
  `_ok`/`_gate`/`_err` helpers; the MCP side of every cross-surface assertion.
- `src/grain/adapters/pypi.py::fetch_latest` (P37-T10) — the network call to monkeypatch.
- CLI commands wired by P37-T04/T08/T09/T10 — driven via `click.testing.CliRunner`.
...
- (This re-verifies the P37-T01 invariant at the integration layer; reuse the value matrix
