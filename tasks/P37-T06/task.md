...
## Dependencies
- **P37-T01** — `domain/envelope.py` (`EngineEnvelope`, `ErrorEnvelope`, `envelope_to_dict`,
...
- **P37-T02** — `grain/version.py::get_version()`. Required for live `serverInfo.version`.
- **P37-T03** — `domain/capabilities.py::CAPABILITIES` + the `Capability` dataclass. Required
  for `McpTool.capability` references and the derived `tools/list`.
- **P37-T05** — `recipe_service.engine_error_to_forge` (recipe engine-error → `ForgeError`).
