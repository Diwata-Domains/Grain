...
`version_check` capability is **seeded and owned by P37-T03** (§6.2) with `surfaces ∋
...
P37-T06 into this packet), each delegating to one shared version-payload service function.
...
single resolver. This packet consumes that resolver (P37-T02), the envelope/error
foundation (P37-T01), the `version_check` capability seed (P37-T03), and the MCP `_ok`
helper + extended `McpTool` (`write`+`capability`, P37-T06) to ship the user-facing
...
- **Register `version` group** in `cli/__init__.py`; reuse `get_version()` (P37-T02) and
...
- **Capability (consume, do NOT re-add):** the `version_check` `Capability` is **seeded
  and owned by P37-T03** in `domain/capabilities.py::CAPABILITIES` (`since: 0.5.0`,
...
- **MCP tool (owned here, moved out of P37-T06):** register the `version_check` MCP
  **read** tool (`write=False`, `capability="version_check"`) using the extended `McpTool`
  + `_ok` helper from P37-T06; it returns the identical payload under the §3 envelope
  (§5.4, §5.7) by delegating to the shared version-payload service function above. P37-T06
...
- `src/grain/cli/version.py` (incl. the shared version-payload service function),
  ...
  `version_check` `Capability` is **consumed from P37-T03's seed, not added here.**
...
- `GRAIN_NO_UPDATE_CHECK=1` makes `grain version --check` perform no network or cache read
  ...
  (§7.2).
- `version_check` is present in `CAPABILITIES` as **seeded by P37-T03** (`since: 0.5.0`,
...
## Dependencies
- **P37-T01** — `domain/envelope.py` (`EngineEnvelope`, `VALID_ENGINE_KINDS` incl.
...
- **P37-T02** — `src/grain/version.py::get_version()`, the single version resolver this
  command and the MCP tool consume for `installed` / `serverInfo.version` (§5.2, §10).
- **P37-T03** — `domain/capabilities.py::CAPABILITIES` with the **`version_check`
...
- **P37-T06** — the MCP `_ok` envelope helper and the extended `McpTool`
...
- `src/grain/domain/capabilities.py` (dependency, P37-T03) — `version_check` `Capability`
...
- `src/grain/domain/envelope.py` (dependency, P37-T01) — frame + error shapes consumed.
- `src/grain/version.py` (dependency, P37-T02) — `get_version()`.
- MCP `_ok` helper + extended `McpTool` (dependency, P37-T06) — consumed to register the tool.
