...
- **Consume, don't redefine:** `EngineEnvelope`/`status` tri-state/serializers come from
  `domain/envelope.py` (P37-T01); the gate emit reuses the CLI envelope/`ctx.obj` seam from
  P37-T04. Do NOT redefine the frame, the `status` vocabulary, or a second emit path.
...
## Dependencies
- **P37-T01** (engine envelope + typed-error foundation) — provides `domain/envelope.py`:
...
- **P37-T04** (envelope CLI wiring + opt-in flag) — provides the CLI emit seam / `ctx.obj`
...
- `src/grain/domain/envelope.py` (dependency, P37-T01) — `EngineEnvelope`/`status` tri-state
  (import, do not modify).
- `src/grain/cli/emit.py` / `src/grain/cli/__init__.py` (dependency, P37-T04) — emit seam +
...
- If routing the always-enveloped gate/result emit (§3.5) appears to require the T04
  ...
  unconditionally for these two sites conflicts with P37-T04's default-bare wiring, record a
